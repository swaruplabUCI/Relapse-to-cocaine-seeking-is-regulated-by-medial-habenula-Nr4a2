...
option_list = list(
  ...
  make_option(
    c('--overwrite'), type='logical', default=FALSE,
...
while(!package_loaded){
  tryCatch(
    ...
    error = function(cond){
      Sys.sleep(15)
      message('Problem with packages, trying again in 15 seconds...')
    }
  )
}
...
out_file <- paste0(opt$outdir, '/', opt$name, '_', cur_group, '.csv')

# do we want to overwrite?
if((!opt$overwrite) & (file.exists(out_file))){
  stop(paste0('Output file already exists and overwrite=FALSE, therefore skipping this group:', cur_group))
}

# get a list of latent vars if present:
if(!is.null(opt$latent)){
  latent = stringr::str_split(opt$latent, ',')[[1]]
} else{
  latent = NULL
}
...
if(opt$type == 'markers'){
  ...
  Idents(seurat_obj) <- seurat_obj@meta.data[,opt$condition]
    ...
    # write to output file:
    write.csv(markers, out_file, quote=FALSE, row.names=FALSE)
