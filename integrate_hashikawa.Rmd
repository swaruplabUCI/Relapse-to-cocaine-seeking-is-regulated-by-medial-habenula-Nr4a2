...
# combine expression matrices of
...
# seurat_meta <- rbind(
#   subset(seurat_joint@meta.data, Dataset == 'Nurr2c'),
#   subset(seurat_joint@meta.data, Dataset == 'Hashikawa')
# )
...
# pdf('figures/liger_umap_integrated_ALS.pdf', width=8, height=7)
...
# have to re-compute harmony in seurat for this to work
seurat_obj <- ScaleData(seurat_obj)
seurat_obj <- RunPCA(seurat_obj)
seurat_obj <- harmony::RunHarmony(seurat_obj,group.by.vars = "Assignment")
seurat_obj <- RunUMAP(seurat_obj, dims=1:30, reduction='harmony', return.model=TRUE)

anchors <- FindTransferAnchors(
    reference = seurat_obj,
    query = seurat_hashikawa,
...
p1 <- DimPlot(seurat_obj, group.by='cell_type') +  hdWGCNA::umap_theme() + theme(plot.title = element_text(hjust = 0.5))
p2 <- DimPlot(seurat_hashikawa, group.by='predicted.cell_type', reduction='ref.umap') +  hdWGCNA::umap_theme() + theme(plot.title = element_text(hjust = 0.5))

pdf(paste0(fig_dir, 'umap_integrated_mapquery.pdf'), width=10, height=5)
p1 + p2
dev.off()
